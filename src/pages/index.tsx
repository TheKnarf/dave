...
import { createInlineTheme } from '../styles/theme';
	...
	const style = useMemo(() => {
		if (typeof window !== 'undefined') {
			const match = window.location.hash.match(
				/^\#([0-9a-f]{3,6})\-([0-9a-f]{3,6})\-([0-9a-f]{3,6})$/i
			);
			if (match !== null) {
				return createInlineTheme({
					background: '#' + match[1],
					text: '#' + match[2],
					accent: '#' + match[3],
				});
		...
		return createInlineTheme(colors);
	}, [typeof window == 'undefined' ? '' : window.location.hash]);

	const bgVar = style['--color-background'];
	const textVar = style['--color-text'];
	const accentVar = style['--color-accent'];

	return (
		<style>{`
:root {
	${bgVar ? `--color-background: ${bgVar};` : ''}
	${textVar ? `--color-text: ${textVar};` : ''}
	${accentVar ? `--color-accent: ${accentVar};` : ''}
}
		`}</style>
	);
};
...
const Home: React.FC<Props> = ({ colors, mdx, appData, forceHttps }) => {
	...
	if (typeof window !== 'undefined') {
		apps = (appData || []).map(({ relativeSubdomain, url, ...app }) => {
	...
	return (
		<>
			<CMDK>
				{(apps || []).map((app) => (
					<Command.Item key={app.id} onSelect={() => gotoApp(app)}>
						{app.name}
					</Command.Item>
				))}
			</CMDK>
			...
			<Style colors={colors} />
			<article>
				<MDX components={{ Grid, App }} scope={{ apps }}>
					{mdx}
				</MDX>
			</article>
		</>
...
`;
	...
	if (process.env.textcolor) colors.text = process.env.textcolor;
	if (process.env.accentcolor) colors.accent = process.env.accentcolor;
